...
await foreach (var user in client.AccountAdminManager.ListProjectUsersAsync(projectId, config =>
{
    config.QueryParameters.Sort = ["name"];
}))
{
...
- Every method accepts optional `requestConfiguration` and `cancellationToken` parameters
